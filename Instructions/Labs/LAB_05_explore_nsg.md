...
在本实验室中，你将探索 Azure 中网络安全组的功能。  你将通过创建不带任何网络安全组 (NSG) 的 VM 来实现此目的。 然后，你将完成创建 NSG 并将 VM 的接口分配给该 NSG 的过程。  配置后，观察默认入站和出站规则并创建新规则。
...
1. 你现在位于用于配置 VM 的“网络”选项卡。  对于“NIC 网络安全组”选项，选择“无”。 将其余所有设置保留为默认值。  注意：在此步骤中选择“无”的目的是在后续任务中逐步完成从头开始创建网络安全组的步骤。
...
创建网络安全组，将 VM 的网络接口分配给该 NSG，并为 RDP 流量创建新的入站规则。
...
1. 在“创建网络安全组”页面的“基本”选项卡上，指定以下设置：
    ...
    1. 选择“查看 + 创建”，然后选择“创建” 。
1. 部署完成后，选择“转到资源”。
1. 请注意 NSG 中的默认入站和出站规则。  尽管已创建 NSG，并且没有用于筛选流量的默认规则，但尚无与 NSG 相关联的接口。
1. 在“NSG-SC900”页面的左侧导航窗格中，选择“设置”下的“网络接口”。
1. 选择搜索框上方的“+ 关联”。
1. 页面右侧是用于选择要关联到 NSG 的网络接口的字段。 选择下拉箭头，然后选择 sc900-winvmXXX（XXX 将特定于 VM 的网络接口），然后在窗口底部选择“确定”。
1. 将接口关联到 NSG 后，它会显示在列表中。
1. 从左侧导航窗格中，选择“入站安全策略”。
1. 默认入站规则拒绝所有非来自 Vnet 或 Azure 负载均衡器的入站流量，因此需要设置规则以允许入站 RDP 流量（端口 3389 上的流量）。 回想一下，你不能删除默认规则，但可通过创建优先级更高的规则来替代默认规则。
1. 在页面顶部选择“添加”：
1. 在“添加入站安全规则”窗口上，指定以下设置：
    1. 源：任何
...
1. 现在，验证是否能够使用 RDP 连接到 VM。  选择页面顶部的搜索字段（在 Microsoft Azure 旁边），查看最近的服务。  选择“虚拟机”。
1. 选择 VM，SC900-WinVM。
1. 从页面顶部选择“连接”，然后从下拉列表中选择“RDP” 。
