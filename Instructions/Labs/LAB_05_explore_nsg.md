<!---
---
Lab: Title: 'Explore Azure Network Security Groups (NSGs)' Learning Path/Module/Unit:'学习路径：描述 Microsoft 安全解决方案的功能；模块 1：描述 Azure 中的基本安全功能；第 6 单元：描述 Azure 网络安全组'
---
--->
...
1. 从页面顶部选择**连接**。  选择 IP 地址下列出的**检查访问**选项。  检查将使用 3389 端口进行，这是 RDP 连接的端口。  你应该会看到无法验证的信息。  在本地 RDP 框中单击**选择**。  在打开的窗口中，在1 配置本地 RDP 的先决条件下，应该会看到Azure 需要配置某些功能才能连接到虚拟机。  在下一个任务中，你将设置 NSG 以明确允许 RDP 连接。
...
1. 在打开的 NSG 标签中，*右键单击*页面顶部的**主页**链接，然后选择**在新选项卡中打开链接**，打开另一个通向 Azure 服务的页面。
...
1. 在“NSG-SC900”页面的左侧导航窗格中，选择“设置”下的“**网络接口**”。
    1. 选择“**关联**”。
    2. 在网络接口关联的字段中，依次选择**向下箭头**、“**sc900-winvmXXX**”，然后在窗口底部选择“**OK**”。 一旦接口与 NSG 关联，它就会显示在列表中。
...
1. 在浏览器上打开 SC900-WinVM - Microsoft Azure 选项卡。 如果之前关闭了浏览器选项卡，请打开新的浏览器选项卡，输入 **https://portal.azure.com**,，然后选择**虚拟机**，再选择虚拟机 **SC900-WinVM**。
