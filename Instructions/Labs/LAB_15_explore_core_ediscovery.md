...
1. “查询条件”页支持你根据可创建的查询为项创建保留。  可以选择使用查询生成器创建查询，或者，对于更高级的用户，使用 KQL 编辑器来创建。 在本练习中，你希望该保留能够保留指定用户在指定位置中的所有内容，因此不会创建查询。
...
设置好保留后，你将创建一个搜索查询。  电子数据展示在搜索完成后执行相关操作，例如导出和下载结果以供将来调查。   备注：与电子数据展示（标准版）案例相关的搜索不会在 Microsoft Purview 门户的“内容搜索”页上列出。 这些搜索仅列在相关电子数据展示（标准版）案件的“搜索”页上。
